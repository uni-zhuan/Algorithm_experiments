...
struct proc{
    int pid;//进程号
    // vector<int> allocationp;//进程已分配到的资源数
    // vector<int> needp;//进程还需要的资源数
    bool free;//该进程是否已经释放所有资源
    // proc(int pid,vector<int> allocationp,vector<int> needp,bool free)
    // {
    //     // this->pid=pid;
    //     // this->free=free;
    //     ;
...
void print()
{
    ...
    cout<<endl<<"Matrix B (request): "<<endl;
    for(int i=0;i<NP;i++)//随机生成资源图分配矩阵和请求矩阵
    {
        for(int j=0;j<NR;j++)
        {
